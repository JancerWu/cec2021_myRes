package etmo.metaheuristics.preserve.mixModel1;
...
//                            0 2 4 6
                        ...
                        childs.add(offSpring[0]);
                        mutation_.execute(childs.get(id));
                    }
...
//                    高斯混合模型选择迁移解
                    ...
                    // init ad model
//                    int vNum = problemSet_.get(doTask).getNumberOfVariables();
//                    double[][] v = new double[populationSize_][vNum];
//                    for (int ii = 0; ii < populationSize_; ii++){
//                        for (int j = 0; j < vNum; j++){
//                            v[ii][j] = population_[doTask].get(ii).getDecisionVariables()[j].getValue() + PseudoRandom.randDouble(-1e-4, 1e-4);
//                        }
//                    }
...
//                        vMean[ii] = StatUtils.mean(mx.getColumn(ii));
//                    }
//
...
//                        int num = problemSet_.get(doTask).getNumberOfVariables();
//                        double[] indiV;
//                        if (maxD <= num){
//                            indiV = new double[maxD];
//                            for (int k = 0; k < maxD; k++){
//                                indiV[k] = childs.get(ii).getDecisionVariables()[k].getValue();
//                            }
//                        }
//                        else {
//                            indiV = new double[maxD];
...
//                                else indiV[k] = childs.get(ii).getDecisionVariables()[k].getValue();
//                            }
//                        }
//                        childs.get(ii).setPdf(adModeal.density(indiV));
//                    }
//
//                    childs.sort(new PdfComparator());

                    for (int cc = 0; cc < 4; cc++){
                        Solution best = childs.get(cc);
                        problemSet_.get(doTask).evaluate(best);
                        evaluations_++;
                        updateReference(best, doTask);
                        updateProblem(best, n % populationSize_, neiborType, doTask);
