...
package etmo.qualityIndicator.util;
			...
			if (invertedFront[i + 1].length == 0)
				...
				if (invertedFront[i + 1].length != invertedFront[0].length) {
					...
					contribution[i] = hypervolume.calculateHypervolume(invertedFront[0], invertedFront[0].length,
							numberOfObjectives) - hypervolume.calculateHypervolume(aux, aux.length, numberOfObjectives);
				} else {
					contribution[i] = hypervolume.calculateHypervolume(invertedFront[0], invertedFront[0].length,
							numberOfObjectives);
				}
	...
	public double[] hvContributions(SolutionSet archive, SolutionSet[] populations) {
		...
		for (int i = 0; i < populations.length; i++) {
			if (invertedFront[i + 1].length == 0)
				 ...
				 * invertedFront[j+1][k]; } }
				 * 
				 * System.out.println(hypervolume.calculateHypervolume(
				 * invertedFront[0], invertedFront[0].length,
				 * getNumberOfObjectives)); System.out.println(index+" "
				 * +aux.length);
				 * System.out.println(hypervolume.calculateHypervolume(aux,
				 * aux.length, getNumberOfObjectives));
				 * 
